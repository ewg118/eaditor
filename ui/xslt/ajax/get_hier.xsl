							...
							<xsl:when test="/content/config/aggregator = 'true'">
								<xsl:choose>
									<xsl:when test="string($lang)">
										<xsl:value-of
											select="count(document(concat($solr-url, '?q=lang:', $lang, '+AND+', if (string($q)) then encode-for-uri($q) else '*:*', '&amp;rows=0&amp;facet.field=', $field, '_hier&amp;fq=', $field, '_hier:%22', encode-for-uri(@name), '%22&amp;facet.prefix=', $next-prefix))//lst[@name='facet_fields']/lst[@name=concat($field, '_hier')]/int)"
										/>
									</xsl:when>
									<xsl:otherwise>
										<xsl:value-of
											select="count(document(concat($solr-url, '?q=NOT(lang:*)+AND+', if (string($q)) then encode-for-uri($q) else '*:*', '&amp;rows=0&amp;facet.field=', $field, '_hier&amp;fq=', $field, '_hier:%22', encode-for-uri(@name), '%22&amp;facet.prefix=', $next-prefix))//lst[@name='facet_fields']/lst[@name=concat($field, '_hier')]/int)"
										/>
									</xsl:otherwise>
								</xsl:choose>
							</xsl:when>
