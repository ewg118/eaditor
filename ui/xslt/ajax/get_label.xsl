...
<!-- 
	...
	function: read the data model extracted from from a web resource and return the label in JSON. see xpl/models/get_model.xpl
-->
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:arch="http://purl.org/archival/vocab/arch#" xmlns:dcterms="http://purl.org/dc/terms/"
	...
	<xsl:template name="get_label">
		<xsl:choose>
			...
			<!-- generic skos:prefLabel handler, works on nomisma.org -->
			...
			<!-- dbpedia handler -->
			<xsl:when test="count(descendant::rdfs:label) &gt; 0">
				<xsl:choose>
					<xsl:when test="string(descendant::rdfs:label[@xml:lang='en'])">
						<xsl:value-of select="descendant::rdfs:label[@xml:lang='en']"/>
					</xsl:when>
					<xsl:otherwise>
						<xsl:value-of select="descendant::rdfs:label[1]"/>
					</xsl:otherwise>
				</xsl:choose>
			</xsl:when>
			<!-- generic dcterms:title handler, works on ANS collection -->
