...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:ead="urn:isbn:1-931666-22-9"
	...
	<xsl:variable name="hasPoints"
		select="boolean(descendant::ead:geogname[string(@authfilenumber) and string(@source)])"/>
	...
	<xsl:template match="mods:modsCollection" mode="root">
		<xsl:apply-templates select="mods:mods" mode="root"/>
	</xsl:template>
		...
		<html>
			<head
				...
				<title id="{$path}">
					...
					<xsl:choose>
						<xsl:when
							test="string(ead:eadheader/ead:filedesc/ead:titlestmt/ead:titleproper)">
							<xsl:value-of
				...
				<xsl:if test="namespace-uri()='http://www.tei-c.org/ns/1.0'">
					<link type="text/css" rel="stylesheet"
						href="http://annotorious.github.com/latest/annotorious.css"/>
