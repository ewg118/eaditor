...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:ead="urn:isbn:1-931666-22-9" xmlns:my="http://example.org/my" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"
	xmlns:xlink="http://www.w3.org/1999/xlink" xmlns:xs="http://www.w3.org/2001/XMLSchema" exclude-result-prefixes="xsl my" version="2.0">
	<xsl:output encoding="UTF-8" method="xml" indent="yes"/>
	<xsl:strip-space elements="*"/>

	<xsl:variable name="agencyCode" select="doc('input:template')//ead:eadid/@mainagencycode"/>
			...
			<xsl:if test="not(*:revisiondesc)">
				<xsl:element name="revisiondesc" namespace="urn:isbn:1-931666-22-9">
					<xsl:element name="change" namespace="urn:isbn:1-931666-22-9">
						...
						<xsl:element name="item" namespace="urn:isbn:1-931666-22-9">Imported into EADitor. Some normalization has occurred to apply EAD 2002 schema validation and conform to EADitor basic requirements.</xsl:element>
					</xsl:element>
				</xsl:element>					
			</xsl:if>
		</xsl:element>
	...
	<xsl:template match="*[local-name()='controlaccess']">
		<xsl:element name="controlaccess" namespace="urn:isbn:1-931666-22-9">
			<xsl:apply-templates select="@*"/>
			<xsl:for-each select="descendant::*[local-name()='corpname'] | descendant::*[local-name()='famname'] | descendant::*[local-name()='function'] | descendant::*[local-name()='genreform'] |
	...
	<xsl:template match="*[local-name()='corpname'] | *[local-name()='famname'] | *[local-name()='function'] | *[local-name()='genreform'] | *[local-name()='geogname'] | *[local-name()='name'] |
