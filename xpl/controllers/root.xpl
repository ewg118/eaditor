...
<p:config xmlns:p="http://www.orbeon.com/oxf/pipeline" xmlns:oxf="http://www.orbeon.com/oxf/processors">
	...
	<!-- attempt to load the base config.xml. If it does not exist, then it has not been created (first run) -->	
	...
	<p:choose href="#url-data-checked">
							...
							<div class="container-fluid">
								<div class="row">
									<div class="col-md-12">
										...
										<p>The base config.xml file does not appear in the eXist-db collection, presumably becaues this is the first time you have run EADitor. Be sure to create
											the 'eaditor-admin' Tomcat role following the <a href="https://github.com/ewg118/numishare/wiki/Tomcat-Authentication" target="_blank">instructions</a> on
		...
		<p:otherwise>
			<p:choose href="#url-data-checked">
				<!-- if the config indicates that EADitor is an aggregation of multiple collections, then display the index generated from the base config -->
				<p:when test="/config/aggregator='true'">
					...
					<p:processor name="oxf:pipeline">
						...
						<p:input name="config" href="../views/pages/index.xpl"/>
						<p:output name="data" ref="data"/>
					</p:processor>
				</p:when>
				<p:otherwise>
					<p:processor name="oxf:unsafe-xslt">
						...
						<p:output name="data" id="collections-list-config"/>
					</p:processor>
					
					<p:processor name="oxf:url-generator">
						<p:input name="config" href="#collections-list-config"/>
						<p:output name="data" id="config"/>
					</p:processor>
